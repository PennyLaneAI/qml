...
##############################################################################
# Although this matrix :math:`A` is not the same as the matrix considered in the Resource cost of naive block encoding section, we can see that exploiting the structure of the matrix has decreased the order of magnitude of the T gate cost from :math:`1 \times 10^{12}` to :math:`3 \times 10^{5}`. 
