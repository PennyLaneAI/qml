...
class QSPKrylovState(QubitizedWalkOperator):
    def __init__(
        self,
        lcu_coeffs,
        lcu_paulis,
        ...
        self.rdm_control_wire =rdm_control_wire
    ...
    def measurement_one(self, obs):
        self.qsp_generic_polynomial()
        return qml.expval(obs)

    def measurement_two(self, obs):
        qml.PauliX(self.rdm_control_wire)
        self.qsp_generic_polynomial()
        return qml.expval(
            __class__.multi_reflection(
                self.rdm_control_wire, set(self.control_wires + self.prep_wires)
            )
            @ obs
        )
...
system_size = 4
def build_circuit(
    angles_even, angles_conj_even, angles_odd, angles_conj_odd
):
    ...
    lambda_value = float(lambda_value.real)
