...
PREP_estimate = AmpNResOp(10)
print(PREP_estimate.resource_decomp(10))

##########################################
# and for SEL, 

SEL_estimate = SelAmpResOp(10)
...
# 
# The identity commutes with the Hamiltonian, so it shifts the spectrum without affecting the dynamics in a time-evolution simulation and can thus be ignored. Note that since this construction shifts the kinetic energy to be centered upon zero, were this block encoding called in a phase estimation the shift would need to be accounted for by undoing the shift classically (similar to how performing phase estimation on the walk operator outputs :math:`\text{arccos}(\lambda)` and this is accounted for classically in most :doc:`qubitized QPE <demos/tutorial_re_for_qubitizedQPE>` workflows).
...
# Since the number of calls to the block encoding in a :doc:`qubitization <demos/tutorial_qubitization>` simulation is directly proportional to the 1-norm, this trick halves the simulation cost of the kinetic operator at almost negligible additional
