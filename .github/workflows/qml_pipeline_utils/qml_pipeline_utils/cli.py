...
def cli_parser():
    ...
    subparsers_build_strategy_matrix = subparsers.add_parser("build-strategy-matrix",
                                                             description="Builds the strategy matrix "
                                                                         "used by GitHub to spawn additional workers")
    add_flags_to_subparser(
        subparsers_build_strategy_matrix,
        "num-workers",
        "examples-dir",
        "glob-pattern"
    )

    subparsers_remove_executable_code = subparsers.add_parser("remove-executable-code-from-extraneous-demos",
                                                              description="Remove executable code from "
                                                                          "sphinx examples-dir demos that are "
                                                                          "not relevant to the current worker")
    add_flags_to_subparser(
    ...
    subparsers_remove_html = subparsers.add_parser("remove-extraneous-built-html-files",
                                                   description="Remove the HTML files and accompanying images/assets "
                                                               "for demos that are not relevant to the current worker")
    add_flags_to_subparser(
    ...
    subparsers_remove_html.add_argument("--preserve-non-sphinx-images", action="store_true",
                                        help="Indicate if static images in the build-dir/_images directory "
                                             "should be deleted or not")
    subparsers_remove_html.add_argument("--gallery-dir-name", type=str, default="demos",
                                        help="The gallery directory name inside build-dir "
                                             "where sphinx puts all gallery demo html files")

    subparsers_clean_sitemap = subparsers.add_parser("clean-sitemap",
                                                     description="Delete html files and remove them from sitemap.xml")
    add_flags_to_subparser(
    ...
    subparsers_clean_sitemap.add_argument("--html-files", type=str,
                                          help="A comma separated list of html files that needs to be deleted "
                                               "from build directory and sitemap.xml")
    ...
    cli_actions = {
        ...
        "remove-extraneous-built-html-files": {
            ...
            "kwargs": {
                ...
                "preserve_non_sphinx_images": getattr(parser_results, "preserve_non_sphinx_images", None),
                "offset": getattr(parser_results, "offset", None),
                "dry_run": getattr(parser_results, "dry_run", None),
                "verbose": getattr(parser_results, "verbose", None),
                "glob_pattern": getattr(parser_results, "glob_pattern", None)
        ...
        "clean-sitemap": {
            ...
            "kwargs": {
                "sphinx_build_directory": Path(getattr(parser_results, "build_dir", "")),
                "html_files_to_remove": list(filter(None, map(str.strip,
                                                              getattr(parser_results, "html_files", "").split(",")))),
                "verbose": getattr(parser_results, "verbose", None),
                "dry_run": getattr(parser_results, "dry_run", None)
            }
        }
    ...
    if parser_results.action not in cli_actions:
        raise ValueError(f"Invalid action '{parser_results.action}'. "
