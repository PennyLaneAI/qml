...
def main():
    ...
    master_automatically_run = set(
        [f for f in master_files if f.startswith("tutorial_")]
    )
    ...
    output_file.write(
        f"Last update: {update_time_str} (All times shown in Eastern time)\n"
    )
    ...
    for filename in automatically_run:
        ...
        if outputs_with_diffs:
            ...
            database_of_differences[filename] = (
                master_outputs,
                dev_outputs,
                outputs_with_diffs,
            )
    ...
    if not demos_with_diffs:
        ...
        for i, demo_name in enumerate(demos_with_diffs):
            master_outputs, dev_outputs, outputs_with_diffs = database_of_differences[
                demo_name
            ]
            ...
            write_file_diff(
                output_file, "Dev", dev_file_url, dev_outputs, outputs_with_diffs
            )
