...
In this demo, this is achieved with parametrized qubit rotations applied to the ground state.

2.
A quantum computation, associated to a unitary :math:`U`, is performed on the system. 
The unitary could be generated by a variational quantum circuit or, more simply, by random circuit as 
proposed in Ref. [1].
...
def quanv(image):
    ...
    for j in range(0, 28, 2):
        for k in range(0, 28, 2):
            ...
            q_results = circuit(phi=[image[j, k, 0], 
                                     image[j, k + 1, 0], 
                                     image[j + 1, k, 0], 
                                     image[j + 1, k + 1, 0]])
...
#
# One can clearly notice the downsampling of resolution and 
...
# and a standard *coross-entropy* loss function.
...
# ^^^^^^^^
#
# We initialize an instance of the model that we train and validate 
# with the dataset that has been already pre-processed by a quantum convolution.
...
# we initialize also "classical" instance of the model that will be directly trained
