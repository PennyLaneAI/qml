...
environment:
	...
	@if [ '$(BASE_ONLY)' = 'true' ]; then\
		...
		if [ '$(UPGRADE_PL)' = 'true' ]; then\
			...
			$$PYTHON_VENV_PATH/bin/python -m pip install --upgrade git+https://github.com/PennyLaneAI/pennylane.git@v0.36.0-rc0#egg=pennylane;\
			...
			$$PYTHON_VENV_PATH/bin/python -m pip install -i https://test.pypi.org/simple/ PennyLane-Catalyst==0.6.0.dev0 --upgrade;\
		fi;\
	fi
