...
-----------------------------------
The first step is to import the :mod:`~.pennylane_qml.qchem.qchem` module.
"""
...
# <https://webbook.nist.gov/chemistry>`_ data base to the structure-data file ``h2o.SDF``.
...
# .. note::
#
#     The atomic structure of the molecule can be read from an input data file with any of the
#     standard formats used in molecular modeling as long as it can be converted to the ``'xyz'``
#     format with `Open Babel <http://openbabel.org/wiki/Main_Page>`_.
#
...
#
# .. image:: ../beginner/quantum_chemistry/water_structure.png
#     :width: 600
#
...
# others [2].
#
# Before launching the HF calculation using the function ``gen_meanfield_data()``, we
...
# *multiplicity* three.
#
# .. image:: ../beginner/quantum_chemistry/hf_references.png
#     :width: 600
#
...
# but the same results can be obtained using ``'psi4'``. Once the calculation is completed,
...
# molecule.
hf_data = qml.qchem.gen_meanfield_data(name, geometry, charge,
...
#
# .. image:: ../beginner/quantum_chemistry/sketch_active_space.png
#     :width: 600
#
# .. note::
...
print("Number of qubits required to perform quantum simulations: {:}".format(2*len(active_indices)))
...
##############################################################################
# results in no doubly-occupied orbitals --- all molecular orbitals are considered to be active.
#
...
##############################################################################
# Finally, the ``qchem`` module provides the :func:`~.generate_hamiltonian`
# function to automate the construction of the electronic Hamiltonian using
# the functions described above.
#
# An example usage is shown below,
qubit_hamiltonian, n_qubits = qml.qchem.generate_mol_hamiltonian(name, 'h2o.SDF', charge, multiplicity,
...
# Hamiltonian.
#
# But this not all, take a look at the tutorial :ref: `pl_qchem_tutorial`
# to see how simple it is to implement the VQE algorithm to compute the ground-state energy of
# your molecule using `PennyLane <https://pennylane.ai/>`__.
#
# .. note::
#     If you have built your electronic Hamiltonian independently by using `OpenFermion
#     <https://github.com/quantumlib/OpenFermion>`__ tools, no problem! The ``qchem`` package
#     provides access to the function :func:`~.convert_hamiltonian` to convert the `OpenFermion
#     <https://github.com/quantumlib/OpenFermion>`__ QubitOperator to a `PennyLane
#     <https://pennylane.ai/>`__ observable.
#
# References
# ----------
#
