...
def build_ansatz(initial_layer_weights, weights, wires, gate_sequence=None):
    ...
    for layer in range(n_layers):
        ...
        for i, wire_pair in enumerate(even_wires):
            ...
            op_list.append(
                gate_sequence[layer, i, 0].item()(weights[layer, i, 0], wires=wire_pair[0])
            )
            op_list.append(
                gate_sequence[layer, i, 1].item()(weights[layer, i, 1], wires=wire_pair[1])
            )
        ...
        for i, wire_pair in enumerate(odd_wires):
            op_list.append(qml.CZ(wires=wire_pair))
            op_list.append(
                gate_sequence[layer, len(wires) // 2 + i, 0].item()(
                    weights[layer, len(wires) // 2 + i, 0], wires=wire_pair[0]
                )
            )
            op_list.append(
                gate_sequence[layer, len(wires) // 2 + i, 1].item()(
