...
class PerturbativeGadgets:
    ...
    def gadgetize(self, Hamiltonian, target_locality=3):
        ...
        computational_qubits, computational_locality, computational_terms = self.get_params(
            Hamiltonian
        )
        ...
        gap = 1
        perturbation_norm = np.sum(np.abs(Hamiltonian.coeffs)) + computational_terms * (
            computational_locality - 1
        )
        ...
        for str_count, string in enumerate(Hamiltonian.ops):
            ...
            for anc_q in range(ancillary_register_size):
                term = qml.PauliX(previous_total + anc_q) @ qml.PauliX(
                    previous_total + (anc_q + 1) % ancillary_register_size
                )
                term = qml.operation.Tensor(
                    term,
                    *string.non_identity_obs[
                        (target_locality - 2) * anc_q : (target_locality - 2) * (anc_q + 1)
                    ]
                )
                obs_pert.append(term)
            coeffs_pert += [l * sign_correction * Hamiltonian.coeffs[str_count]] + [l] * (
                ancillary_register_size - 1
            )
    ...
    def get_params(self, Hamiltonian):
        ...
        computational_locality = max(
            [len(Hamiltonian.ops[s].non_identity_obs) for s in range(computational_terms)]
        )
    ...
    def run_checks(self, Hamiltonian, target_locality):
        ...
        if computational_qubits != Hamiltonian.wires[-1] + 1:
            raise Exception(
                "The studied computational Hamiltonian is not acting on "
        ...
        if len(np.unique(localities)) > 1:
            raise Exception(
                "The given Hamiltonian has terms with different locality."
        ...
        if int(ancillary_register_size) != ancillary_register_size:
            raise Exception(
                "The locality of the Hamiltonian and the target"
