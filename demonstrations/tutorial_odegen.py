...
In this demo we introduce ODEgen, a method to compute gradients of pulse programs on quantum computers.
...
.. note::
    ...
    For a loss function :math:`L(\theta) = \langle 0 | U(\theta)^\dagger H_obj U(\theta)` with a gate :math:`U(\theta) = e^{-i \frac{\theta}{2} H}` generated by 
    ...
    For the case of multiple gates generated by different Paulis :math:`H_j` with different parameters :math:`\theta_j`, :math:`U(\vec{\theta}) = \prod_j e^{-i \frac{\theta_j}{2} H_j}`,
    the rule still has the same form

    .. math:: \frac{\partial}{\partial \theta_j} L(\vec{\theta}) = \frac{1}{2} \left(L(\vec{\theta} + \vec{e}_j\frac{\pi}{2}) - L(\vec{\theta} - \vec{e}_j \frac{\pi}{2}) \right)

    such that the shifts of :math:`\pm \frac{\pi}{2}` are only applied to differentiated gates via the basis vectors :math:`\vec{e}_0 = (1,0,0,..)`, :math:`\vec{e}_1 = (0,1,0,..)` etc.
...
shifted evolutions :math:`U(T, \tau) e^{-i\left(\pm\frac{\pi}{4}\right)H_i} U(\tau, 0) |0 \rangle`. As an exception, we explicitly state the evolution times and use the notation :math:`U(t_1, t_0)`
...
In practice, the integral is approximated via Monte Carlo integration
 ...
 = \langle \psi_q^\pm(\tau) | H_\text{obj} | \psi_q^\pm(\tau) \rangle
