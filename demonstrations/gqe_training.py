...
application for ground state search". We will only train on a fixed training dataset using the molecular data  
available in `PennyLane Datasets <https://pennylane.ai/datasets/>`__ as opposed to learning directly from the data 
generated by the model (to be called online training afterwards). It is briefly outlined in the conclusion on how to do
online training and only minor changes to the training loop code needs to be changed for it.
Lastly, we will show that the model gradually better approximates the correct energies and, in turn, 
...
.. figure:: ../_static/demonstration_assets/gqe_training/paper_vqe_diagram.png
    ...
    Figure 1 from [#nakaji2024]_

There are some issues with VQE scalability [#nakaji2024]_, however. This shortcoming makes it less competitive against
...
# transformer (GPT) architecture [#radford2019], [#vaswani2017]. So, a GQE using a transformer is called GPT-QE. 
...
# of operators :math:`U_j`'s loaded from `PennyLane Datasets <https://pennylane.ai/datasets/>`__. We interpret these integers as 
...
#    :width: 90%
# 
#    Overview for offline training of GPT-QE
...
# the full sequence of three operators, which was described in the paper. This can be done simply in PennyLane, using 
...
# correct energies and in turn, correctly sample sequences with lower energies.
# 
...
#    :width: 90%
# 
#    Figure 1: The average subsequence loss for each training iteration

##############################################################################
# We see in Figure 1 that the average loss
...
#    :width: 90%
# 
#    Figure 2: True and predicted energies for sequences generated by the GPT model for each 500th training iteration
