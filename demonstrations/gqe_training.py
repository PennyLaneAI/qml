...
# 
#    -  We returned all the logits generated by the network instead of
...
#       last logit was used to correspond to the next token for each forward pass of the network.
#    -  We defined the ``calculate_loss()`` as a function of the error between the total logits of the sequence
...
#       also introduce numerical instabilities because the numbers would be unnecessarily magnified exponentially.
# 
...
# to the ground state energy ``grd_E=-1.1372633205048763``. But we notice that the maximum energy
