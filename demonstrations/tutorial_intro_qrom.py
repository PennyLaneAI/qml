...
You will also see how easy it is to use this new operator through the :class:`~.pennylane.QROM` template.
...
For example, suppose our data consists of four bit-strings, each with two bits: :math:`[011, 101, 111, 100]`. Then, the index register will consist of two
...
# In this way, we reduce the number of operators that Select requires. The main idea is to organize the data in two dimensions, with each bitstring labelled by a column index :math:`c` and a row index :math:`r`.
...
# the target wires.
#
# Let's look at an example. Let's suppose that we want to load in the target wires the bitstring with the index :math:`5`.
# For it, we put as input in the control wires the input :math:`|101\rangle`, where the first two bits refer to the
# index :math:`c = |10\rangle` and the last one to the index :math:`r = 1`.  After applying the Select block, we
# obtain :math:`|101\rangle|b_4\rangle|b_5\rangle`, loading the column :math:`c`.
# Since the third
# control qubit (:math:`r`) is a :math:`|1\rangle`, it will activate the swap block, generating the state :math:`|101\rangle|b_5\rangle|b_4\rangle`
...
#    :target: javascript:void(0)
#
# In this example, we have that :math:`c = |1\ranlge` and :math:`r = |01\rangle`. We encourage you to check that :math:`b_5`
# indeed appear in the target register.
#
#
# Reusable qubits
# ~~~~~~~~~~~~~~~~~
