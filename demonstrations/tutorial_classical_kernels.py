...
test_features = np.asarray([1.0 / (1 + i) for i in range(2**n_wires)])
...
for step in range(max_steps):
    ...
    if (step + 1) % 10 == 0:
        print(
            "Error norm at step {0:3}: {1}".format(
                step + 1, np.linalg.norm(F(probabilities, spectrum))
            )
        )
...
def probabilities_threshold_normalize(probabilities, thresh=1.0e-10):
    d = len(probabilities)
