...
The nice thing about this representation is that we can see that there is no entanglement in the result and you could somehow prepare each qubit independently. Therefore, what we will do is to create these operators one by one. We will call them $U_k$ and they will be defined as follows:
...
This operator can be easily built with controlled Phase Shift gates that add a phase only to $|1\rangle$. We will represent these gates with a box in which we will include the phase they add. Below we show an animation in which this operator can be visualized for the particular case of $n = 4$ and $k = 1$.
...
.. figure:: ../_static/demonstration_assets/qft/qft3.jpeg
