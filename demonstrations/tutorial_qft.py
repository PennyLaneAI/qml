...
where :math:`|x\rangle = |x0 \dots x_k\rangle \otimes |x_{k+1} \dots x_{n-1}\rangle`.
...
As can be seen, each qubit adds a phase to the k-th qubit, proportional to its position in the binary representation. Note that qubits over the :math:`k`-th one don't contribute to the k-th qubit since they would make complete turns and it would not affect their value.
...
That is why we should make a last step and change the order of all the qubits. That is why it is common to find some swap operators at the end of the template.
"""
