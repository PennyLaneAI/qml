...
E = plt.subplots()[1]
E.plot(range(n), E_vqe - E_fci, "go-", ls="dashed")
E.plot(range(n), np.full(n, 0.001), color="red")
...
# illustrate how it converges to the FCI equilibrium bond length
# :math:`d = 0.986 \AA`.

d_fci = 0.986
d = plt.subplots()[1]
d.plot(range(n), bond_length, "go-", ls="dashed")
d.plot(range(n), np.full(n, d_fci), color="red")
...
# molecule.
#
# The animated figure below uses stored snapshots of the atomic structure of the 
...
# molecule that corresponds to an equilateral triangle with the H atoms
