...
######################################################################
# The idea behind this circuit is that QSVT encodes the desired polinomial :math:`p(x)` but also
# a polynomial :math:`i q(x)`. To isolate :math:`p(x)` we have made use of an auxiliary qubit and the property that
