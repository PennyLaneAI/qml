...
# -------------------------------------------
#
# As a warmup exercise we implement a T-gadget in PennyLane.
# A T-gadget realizes a ``T`` gate using a "magic" input state, a ``CNOT``
...
def t_gadget(target_wire, aux_wire):
    ...
    mcm = qml.measure(aux_wire, reset=reset)
    qml.cond(mcm, qml.S)(target_wire)


######################################################################
# Run a circuit that compares ``qml.T`` and our T-gadget.
#
...
# --------------------------------------------------------
#
# Define some quantum subprograms: two blocks
...
######################################################################
# Define two functions that process MCMs into a boolean condition
...
######################################################################
# Define a shot-based device and a Hamiltonian to be measured.
#
...
######################################################################
# Create a QNode that executes blocks of quantum gates interleaved with layers of
# mid-circuit measurements.
# The MCMs are either processed into a condition for whether the next block is
# applied or used to postselect.
#
