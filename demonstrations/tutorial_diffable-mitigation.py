...
Differentiable transform ``qml.transforms.mitigate_with_zne``
--------------------------------------------------------------
...
# Setting ``grouping_type="qwc"`` optimizes the measurement procedure by grouping the Hamiltonian terms into qubit-wise-commuting (qwc) terms.
H = qml.Hamiltonian(coeffs, observables) #, grouping_type="qwc") #bug!
...
qnode_mitigated = mitigate_with_zne(scale_factors, fold_global, richardson_extrapolate)(
    qnode_noisy
)
