...
dev_stochastic_1 = qml.device("lightning.qubit", wires=num_wires, shots=1)
qnode_stochastic_1 = qml.QNode(circuit, dev_stochastic, interface="autograd")
...
for _ in range(steps):
    cost_SGD1.append(qnode_stochastic_1(params_SGD1))
    params_SGD1 = opt.step(qnode_stochastic_1, params_SGD1)

# Optimizing using stochastic gradient descent with shots=100

dev_stochastic_100 = qml.device("lightning.qubit", wires=num_wires, shots=100)
qnode_stochastic_100 = qml.QNode(circuit, dev_stochastic, interface="autograd")
...
for _ in range(steps):
    cost_SGD100.append(qnode_stochastic_100(params_SGD100))
    params_SGD100 = opt.step(qnode_stochastic_100, params_SGD100)
...
qnode = qml.QNode(circuit, dev_stochastic_100, interface="autograd")


def loss(params):
    return 4 + (5 / 1) * qnode(params, n=1)
...
for i in range(250):
    ...
    dev = qml.device("lightning.qubit", wires=num_wires, shots=int(1 + (n - 1) ** 2))
    qnode = qml.QNode(circuit, dev, interface="autograd")


    def loss(params):
        return 4 + (5 / n) * qnode(params, n=n)
