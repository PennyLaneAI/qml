...
for i_wires in n_wires_ls:
    ...
    def circuit(params):
        qml.StronglyEntanglingLayers(params, wires=range(i_wires))
...
ax1.set_xticks(n_wires_ls)
ax1.set_ylabel("Log Time")
ax1.set_yscale("log")
ax1.set_title("Scaling with wires")

ax2.plot(n_layers_ls, t_exec_layers, '.-', label="circuit")
ax2.plot(n_layers_ls, t_grad_layers, '.-', label="gradient")
