...
.. figure:: ./images/class_shadow_ml.png
...
nx.draw(G, pos, node_color='lightblue', with_labels=True, node_size=600, 
        width=weights, edge_color="firebrick")
...
for i, j in coups:
    ...
    if i == j:
        ...
        expval_exact[i][j] = np.sum(np.array([circuit_exact(psi0, observable=[o]) 
                                              for o in corrs]).T)/3
...
######################################################################
# .. figure:: ./images/class_shadow_prep.png
...
def snapshot_state(meas_list, obs_list):
    ...
    rotations = [qml.Hadamard(wires=0).matrix, # X-basis
...
# exact.
# 
# .. figure:: ./images/fidel_snapshot.png
...
def estimate_shadow_obervable(shadow, observable, k=10):
    ...
    for i in range(0, shadow_size, shadow_size // k):
        meas_list_k, obs_lists_k = (meas_list[i : i + shadow_size // k], obs_lists[i : i + shadow_size // k],)
        indices = np.all(obs_lists_k[:, target_locs] == target_obs, axis=1)
        if sum(indices):
            means.append(np.sum(np.prod(meas_list_k[indices][:, target_locs], axis=1)) / sum(indices))
        else:
            means.append(0)
...
for i, j in coups:
    ...
    if i == j:
        ...
        expval_estmt[i][j] = np.sum(np.array([estimate_shadow_obervable(shadow, o, k=k+1) 
                                                      for o in corrs]))/3
    ...
    for idx in tqdm(range(num_points), leave=False):
        ...
        for i, j in coups:
            ...
            if i == j:
                ...
                expval_exact[i][j] = np.sum(np.array([circuit_exact(psi, observable=[o]) 
                                                      for o in corrs]).T)/3
                expval_estim[i][j] = np.sum(np.array([estimate_shadow_obervable(shadow, o, k=k+1) 
                                                      for o in corrs]))/3
...
def fit_predict_data(cij, kernel, opt="linear"):
    ...
    models = [(lambda Cx: svm.SVR(kernel=opt, C=Cx, epsilon=0.1)),  # Epsilon-Support Vector Regression
              (lambda Cx: KernelRidge(kernel=opt, alpha=1/(2*Cx)))] # Kernel-Ridge based Regression
    hyperparams = [0.0025, 0.0125, 0.025, 0.05, 0.125, 0.25, 0.5, 1.0, 5.0, 10.0] # Regularization parameter
    best_model, best_pred, best_cv_score, best_test_score = None, None, np.inf, np.inf
    for model in models:
        for hyperparam in hyperparams:
            cv_score = -np.mean(cross_val_score(model(hyperparam), X_train, y_train, cv=5, 
                                                scoring="neg_root_mean_squared_error"))
            if best_cv_score > cv_score:
                best_model = model(hyperparam).fit(X_train, y_train)
                best_pred = best_model.predict(X_test)
                best_cv_score = cv_score
                best_test_score = np.linalg.norm(best_model.predict(X_test).ravel() - 
                                                 y_test_clean.ravel()) / (len(y_test) ** 0.5)

    return best_model, best_pred, y_test_clean, np.round(best_cv_score,5), np.round(best_test_score,5), 
...
for cij in tqdm(range(num_qubits**2), leave=False):
    ...
    kernel_data[cij][0] = (best_cv_score, test_score); y_predicts1.append(y_predict)
    clf, y_predict, y_clean, best_cv_score, test_score = fit_predict_data(cij, kernel_dirichlet)
    kernel_data[cij][1] = (best_cv_score, test_score); y_predicts2.append(y_predict) 
    clf, y_predict, y_clean, best_cv_score, test_score = fit_predict_data(cij, kernel_NN)
    kernel_data[cij][2] = (best_cv_score, test_score); y_predicts3.append(y_predict)
...
for idx, data in enumerate(kernel_data):
    print(row_format.format(f"C_{idx//num_qubits}{idx%num_qubits} \t| ", 
...
cols = ['From {}'.format(col) for col in ['Exact Diagnalization', 'RBF Kernel', 
...
for itr in range(3):
    for idx, corr_val in enumerate(corr_vals):
        shw = axes[itr][idx].imshow(np.array(corr_vals[idx]).T[plt_plots[itr]].reshape(Nr*Nc, Nr*Nc), 
...
######################################################################
# .. figure:: ./images/rmse_shadaow.png
#     :width: 48 % 
#
# .. figure:: ./images/rmse_training.png
#     :width: 48 %
#
