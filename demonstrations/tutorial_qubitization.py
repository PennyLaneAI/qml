...
Given the effect that :math:`\text{PSP}_{\mathcal{H}}` has on the state :math:`|0\rangle|\phi\rangle`, it can be verified that the second reflection occurs along the line that bisects the angle between the state :math:`|\Psi\rangle` and :math:`|0\rangle |\phi\rangle`. Let's now examine the effect this reflection has on the previous state:
...
After applying the reflection, the new state is rotated by :math:`\theta` degrees relative to the initial state :math:`|\Psi\rangle`. This shows that the Qubitization operator successfully creates a rotation of :math:`\theta` degrees within the subspace.
