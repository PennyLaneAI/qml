...
==========================================================
 ...
 .. meta::
    ...
    Quantum Phase Estimation (QPE) algorithm and shows users how to implement a
...
# to evolve for a time :math:`t`. The time-dependent Schrödinger equation
...
# matters to an extent. We will demonstrate later how this inaccuracy
...
# :math:`\hat{H}` that generates :math:`\hat{U}`. Perhaps, you can already guess that we will encode the
...
#
#  where :math:`\delta := \varphi_u - \tilde{\varphi}_{u} < 2^{-K}.` In general, there is more than one non-zero
...
# be sure that we measured something *relevant*? We agree,
