...
print(np.allclose(local_inversion(), np.array([[1.0, 0.0], [0.0, 0.0]])))
...
def sewing_2():
    ...
    return (
        qml.density_matrix(wires=[2]),
        qml.density_matrix(wires=[n]),
        qml.density_matrix(wires=[n + 1]),
    )
...
def sewing_3():
    ...
    return (
        qml.density_matrix(wires=[3]),
        qml.density_matrix(wires=[n]),
        qml.density_matrix(wires=[n + 1]),
        qml.density_matrix(wires=[n + 2]),
    )
...
for i in range(n):
    ...
    print(
        f"Bloch sphere coordinates of qubit {i} after inversion: {X_res:.5f}, {Y_res:.5f} {Z_res:.5f}"
    )
