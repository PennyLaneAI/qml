...
def cli(c):
    ...
    if c:
        ...
        start = index + 1
        default_qubit_demos = default_qubit_demos[start:]

    lightning_qubit_demos = np.array(["lightning_" + default_qubit_demos[i].replace(".py\n", "") for i in range(num_demos - start)])
    for i, (default_demo, lightning_demo) in enumerate(zip(default_qubit_demos, lightning_qubit_demos)):
        ...
        df = pd.DataFrame({"Tutorial": default_demo, "Default runtime": runtimes[0], "Lightning runtime": runtimes[1], "errors": error_msg}, index=[i + start])
