...
One of the recursion steps will produce a Cosine-Sine Decomposition (CSD), maybe the
...
</demos/tutorial_fixed_depth_hamiltonian_simulation_via_cartan_decomposition>` [#Kökcü_FDHS]_.
...
To understand the nature of :math:`mathcal{A}`, consider the Lie algebra :math:`mathfrak{g}` of :math:`mathcal{G}` with
the algebra :math:`mathfrak{k}` of :math:`mathcal{K}` being a subalgebra. The orthogonal complement of :math:`mathfrak{k}`
within :math:`mathfrak{g}` is the so-called *horizontal space*, :math:`mathfrak{p}`. A *(horizontal) Cartan
subalgebra* :math:`mathfrak{a}` then is any maximal Abelian subalgebra of :math:`mathfrak{p}` and the Cartan
subgroup :math:`mathcal{A}` simply is the (Abelian) group generated by :math:`mathfrak{a}`.

Note that there are (infinitely) many possible choices for :math:`mathfrak{a}` and thus for :math:`mathcal{A}`,
and for each choice, the KAK decomposition exists.
...
In the following, we will exclusively use Cartan decompositions of type AIII or of type A, with
...
an element from :math:`mathcal{K}` can be drawn as

??? TODO: Drawing embedding?
...
If :math:`V` denotes the diagonalizing operator of :math:`Y`, we may draw this as
...
which is exactly of the form of :math:`\mathfrak{a}_{\text{AIII}}` except for the diagonalization 
:math:`V` on the first qubit that we used above.
We thus may draw the type-A decomposition using :math:`\mathfrak{a}_{\text{A}}` as
...
Tab. 1 of [#Shende_QSD]_ by decomposing :math:`U(4)` with a standard 3-CNOT circuit as used
for `two-qubit synthesis <https://pennylane.ai/compilation/two-qubit-synthesis>`__, see
