...
Shadow Hamiltonian simulation can be viewed as :doc:`demo on g-sim </demos/tutorial_liesim>`
[#Somma]_ [#Somma2]_ [#Galitski]_ [#Goh]_ but run on a quantum computer with a single, specific gate generated by :math:`H`.
...
we only have one specific "gate", we do not need the full Lie closure of but it is sufficient to choose :math:`O_m` such that they
support all :math:`[H, O_m]`.
