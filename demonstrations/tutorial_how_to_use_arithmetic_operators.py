...
# Now we can implement and example for the :class:`~.pennylane.Adder`. We will add the constant term :math:`4` to the output register:

@qml.qnode(dev)
def circuit():

    qml.BasisState(0, wires=wires["output"]) #     |0>
    qml.Adder(4, wires["output"])            #     |0+4>
...
######################################################################
# We obtained the result :math:`4`, as expected.
#
...
# The class :class:`~.pennylane.Multiplier` performs an **Inplace** operation, multiplying the state of the wires :math:`|w \rangle` by a constant value :math:`k`. It is defined as:
#
# .. math::
#
...
#
# First, let's see an example for the :class:`~.pennylane.Multiplier` operator. We will multiply the state of the wire :math:`|x \rangle=|1 \rangle` by the constant term,
...
# This is possible through the :function:`~.pennylane.adjoint` operation, which allows us to reverse a multiplication or addition operation, hence enabling
...
# the :class:`~.pennylane.Multiplier` and :class:`~.pennylane.OutMultiplier` operators for this.
...
######################################################################
# Cool, we get the correct result :math:`33`!
