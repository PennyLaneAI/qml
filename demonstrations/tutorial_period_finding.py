...
However, group theory is a huge hurdle for even some of the more seasoned quantum enthusiasts. This demo wants to give a glimpse of what this "Abelian structure" is all about. Luckily, the fruit-fly example of a hidden subgroup problem is just the task of finding the period of a integer-valued function - something one can appreciate without much group jargon. A fantastic resource to read up on the basics is the review of hidden subgroup problems by Childs & Dam (2010) [#Childs2010]_.
"""
...
# values :math:`\{0,4,8\}` form a *subgroup* that is "generated" by the period 4. Finding the period
...
# the subgroups, which are shifted sets of the form :math:`\{0+s, 4+s, 8+s\}` (where :math:`s` is an
...
# interpreted as integers :math:`\{0, 3, 6, 9\}`. By sampling only two distinct values, say 6 and
...
#
# Since we move between integers and computational basis states, we need these utility conversion functions.
...
# together by defining a unitary matrix that does the job. Of course, in a real application this 
...
# reason we define a device with 2 shots. We also add some snapshots to the circuit that we will look at below.
...
#####################################################################
# The classical postprocessing for period finding is relatively simple, while 
# for more general hidden subgroup problems it requires more algebra. 
