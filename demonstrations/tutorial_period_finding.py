...
# #################
#
# Consider a function :math:`f(x)` that maps from integers, say the numbers between :math:`0` and :math:`11`, to
# some other domain, such as the numbers between :math:`0` and :math:`3`. The function is guaranteed to have a
...
#    :width: 90%
#
# Importantly, we assume that we have *black-box access* to that function, which sounds abstract,
# but can just be thought of someone giving you a python function `f` that takes integers
# :math:`x \in \ {\0,...,11\}`, but you have no access to the definition `def f(x)` --- your only way to
...
# so-called *cyclic group*, which is an *Abelian* group that Aaronson referred to above. The
# values :math:`\{0,4,8\}` are a so-called *subgroup* "generated" by the period 4. Finding the period
...
# the subgroups, which are "shifted" versions of the form :math:`\{0+s, 4+s, 8+s\}` (where :math:`s` is an
# element of the group). This jargon hopefully does not scare you, but should illustrate that
...
# interpreted as integers :math:`\{0, 3, 6, 9\}`. By sampling only two distinct values, say :math:`6` and
# :math:`9`, we can determine the common denominator as :math:`3`, which is the number of periods fitting
# into :math:`12`. From there we can recover :math:`12/3 = 4`.
#
# We'll now implement this recipe, but use a more convenient cyclic group of :math:`16` elements
# :math:`\{0,...,15\}`, which can be exactly represented by 4 qubits.
...
# unknown in principle, but we can call it on a classical computer, and --- by turning the
# classical logical circuit into a reversible quantum circuit --- on a quantum computer. The call
...
# nonzero uniform amplitudes at :math:`| 0010 \rangle | 11 \rangle`, but zero amplitudes at the states
# :math:`| 0010 \rangle | 00 \rangle`, :math:`| 0010 \rangle | 01 \rangle`, :math:`| 0010 \rangle | 10 \rangle`.
#
# Since we move between integers and computational basis states, we need a few utility functions
...
# :math:`\sum_x |x \rangle |f(x) \rangle`. There are many such unitaries and we'll hack it somewhat
# together. Of course, in a real application the unitary would emerge from translating a
# classical algorithm for `f` into a sequence of quantum gates.
...
# quantum routine and a classical postprocessing step. The quantum part is simple: prepare the
...
#
# We only have to get two unique samples, from which we can compute the period of the function.
#
# .. note: We add some snapshots to the circuit that we will look at below.
...
@qml.qnode(dev)
def circuit():
    """Circuit to implement the hidden subgroup problem algorithm."""
...
# look how it acts on a superposition of inputs :math:`x` for which :math:`f(x)` is the same, such as
...
for k in range(12):
    ...
    print(f"k={k} --- {np.abs(res)**2}")

#####################################################################
# This pattern is the same for whichever :math:`x` with the same value :math:`f(x)` we picked. It is also
