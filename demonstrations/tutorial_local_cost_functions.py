...
# Letâ€™s use a more likely number of qubits: n=10. We will say that after
...
for runs in range(samples):
  ...
  params_global = [[np.random.uniform(-np.pi, np.pi) for i in range(wires)], [np.random.uniform(-np.pi, np.pi) for i in range(wires)]]
  for i in range(steps):
      # update the circuit parameters
      params_global = opt.step(cost_global, params_global)
...
for runs in range(samples):
  ...
  print("--- New run! ---")
  has_been_trained = False
  params_tunable = [[np.random.uniform(-np.pi, np.pi) for i in range(wires)], [np.random.uniform(-np.pi, np.pi) for i in range(wires)]]
  for i in range(steps):
    # update the circuit parameters
    params_tunable = opt.step(cost_tunable, params_tunable)
