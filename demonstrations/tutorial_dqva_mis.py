...
# :math:`H_C`, and a *mixer Hamiltonian*, :math:`H_M`, to evalulate the
...
# using QAOA, constraints can be imposed in two ways: 1. By adding a
# penalty term to the problem's objective function (the Lagrange
# multiplier approach) 2. By constructing the variational ansatz in a way
# such that constraints are satisfied at all times
# 
...
# share an edge. A *Maximum Independet Set* (MIS) is an independent set of
...
# This extra qubit is used as an ancilliary qubit to serve as the target
...
def mixer_layer(beta, ancilla, mixer_order):
    ...
    for qubit in list(mixer_order):
        ...
        qml.MultiControlledX(control_wires = ctrl_qubits,
                             wires = [ancilla], 
                             control_values ='0'*len(neighbors))
...
def solve_mis_qaoa(init_state, P=1, m=1, mixer_order=None,  threshold=1e-5, cutoff=1):
    ...
    def f(params):
        probs = probability_circuit(P, params=params, init_state=cur_init_state, mixer_order=cur_permutation)
        avg_cost = 0
    ...
    for mixer_round in range (1, m+1):
        ...
        while (inner_round < 2):
            print('Start round {}.{}, Initial state = {}'.format(mixer_round, inner_round, cur_init_state))
            ...
            # Obtain probabilites
            probs = probability_circuit(P, params=opt_params, init_state=cur_init_state, mixer_order=cur_permutation)
            top_counts = list(map(lambda x: np.binary_repr(x, len(graph.nodes)), np.argsort(probs)))[::-1]
            # print (top_counts)
...
# set, but this may not be the maximum. Let's try to run with with a few
...
# 
# The dynamic ansatz update includes: 1. Optimization of parameters using
# Gradient Descent and finding the Hamming weight with new parameters 2.
# If the Hamming weight of the new state is larger than the initial state
# , the initial state gets updated to this new state 3. Based on this new
# state, partial mixers are updated (i.e., turned off for ones and turned
# on for zeros) 4. Steps 2 and 3 are repeated until the Hamming weight can
# no longer be improved 5. If no new Hamming weight is obtained, the
# partial mixers are randomized and steps 2 and 3 are repeated to check if
# a better Hamming weight is found. The number of randomizations is
# controlled via a hyperparameter.
# 
...
def solve_mis_dqva(init_state, P=1, m=1, mixer_order=None, threshold=1e-5,cutoff=1):
    ...
    def f(params):
        ...
        probs = probability_dqva(P, params=params, init_state=cur_init_state, mixer_order=cur_permutation)
        avg_cost = 0
        for sample in range(0, len(probs)):
    ...
    for mixer_round in range (1, m+1):
        ...
        while True:
            ...
            probs = probability_dqva(P, params=opt_params, init_state=cur_init_state, mixer_order=cur_permutation)
...
######################################################################
# We also run the algorithm for a number of initial states (instead of
