...
For that, we look at an expectation vector :math:`\vec{e}` made up of expectation values with respect to some transformed initial state :math:`\rho`,

.. math:: (\vec{e})_\alpha = \text{tr}\left[ h_\alpha U \rho U^\dagger \right] = \text{tr}\left[ U^\dagger h_\alpha U \rho \right].
...
.. math:: e^{i \theta h_\mu} h_\alpha e^{-i \theta h_\mu} = \sum_\beta e^{\theta f^\mu_{\alpha \beta}} h_\beta.
...
We are interested in expectation values of observables composed of DLA elements,

.. math:: \langle \hat{O} \rangle = \text{tr}\left[\hat{O} \mathcal{U} \rho_0 \mathcal{U}^\dagger \right]

with :math:`\langle \hat{O} \rangle = \sum_\alpha \omega_\alpha h_\alpha` and the unitary evolution of a parametrized quantum circuit generated by DLA elements,
...
vector in Lie-sim, which is more efficient whenever :math:`\text{dim}(\mathfrak{g}) < 2^n`. And, in particular, generally efficient, 
...
# We take for simplicity the one dinemsional variant and define its generators and compute the :func:`~pennylane.lie_closure`.
