...
these effects (e.g., electronic correlation energy) are typically a tiny fraction
of the total energy of the molecule [#jensenbook]_.
...
# field generated by all other electrons in the molecule [#pople1977]_. The optimized coefficients
# are precisely the molecular orbitals we need to build the second-quantized Hamiltonian.
#
...
# orbitals the elements of this basis are labeled as
...
# Coulomb integrals [#ref_integrals]_ evaluated using the Hartree-Fock orbitals.
#
...
# the :func:`~.pennylane_qchem.qchem.decompose` function. This function
...
# file ``'./pyscf/sto-3g/water.hdf5'``. Then, it builds the fermionic Hamiltonian and
# map it to the qubit representation.
...
# function can also be used to automate the construction of the electronic
# Hamiltonian using the functionalities described above. An example usage
# is shown below:

H, qubits = qchem.molecular_hamiltonian(symbols, coordinates, name="water")
...
# determinant resulting from occupying the five lowest-energy orbitals with two
# *paired* electrons per orbital, defines a closed-shell HF state with spin
# **multiplicity** one. Alternatively, if we define an occupation where the first four orbitals
# are doubly occupied and the next two are singly occupied by *unpaired* electrons, this
# corresponds to an open-shell HF state with **multiplicity** three.
#
# |
#
...
# of four electrons and four active orbitals using
